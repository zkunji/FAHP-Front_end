...
export default {
  data() {
    return {
      tableData: Array.from({ length: 10 }, (_, rowIndex) => {
  ...
  methods: {
    ...
    async setDefaultValues() {
      this.tableData = Array.from({ length: 10 }, () => Array(10).fill(1))
    ...
    async collectAndSubmit() {
      ...
      if (userStore.resultFlag[9] === true) {
...
<template>
  ...
  <el-row class="resultFlag">
    <el-col v-if="userStore.resultFlag[9]">
