...
export default {
  data() {
    return {
      tableData: Array.from({ length: 4 }, (_, rowIndex) => {
  ...
  mounted() {
    ElMessageBox.confirm('您是否使用上次保存的数据？', '温馨提示', {
      ...
      .then(() => {
        const arrayData = getSecondArray().value
        console.log(arrayData)
        if (this.isTwoDimensionalEmptyArray(arrayData)) {
  ...
  methods: {
    isTwoDimensionalEmptyArray(arr) {
    ...
    saveData() {
      const collectedData = this.tableData.slice(0).map((row) => row.slice(0))
      setSecodeArray(collectedData)
    ...
    async setDefaultValues() {
      this.tableData = Array.from({ length: 4 }, () => Array(4).fill(1))
    ...
    async collectAndSubmit() {
      ...
      if (userStore?.resultFlag[1] === true) {
...
<template>
  ...
  <el-row class="resultFlag">
    <!-- :sm="8" :lg="6" -->
    <el-col v-if="userStore?.resultFlag[1]">
  ...
  <div class="container">
    ...
    <el-button type="success" @click="collectAndSubmit">一致性测试</el-button>
