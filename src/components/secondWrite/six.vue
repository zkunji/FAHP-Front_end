...
export default {
  data() {
    return {
      tableData: Array.from({ length: 3 }, (_, rowIndex) => {
  ...
  methods: {
    ...
    async setDefaultValues() {
      this.tableData = Array.from({ length: 3 }, () => Array(3).fill(1))
    ...
    async collectAndSubmit() {
      ...
      if (userStore.resultFlag[6] === true) {
...
<template>
  ...
  <el-row class="resultFlag">
    <el-col v-if="userStore.resultFlag[6]">
