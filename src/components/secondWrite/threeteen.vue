...
export default {
  data() {
    return {
      tableData: Array.from({ length: 6 }, (_, rowIndex) => {
  ...
  methods: {
    ...
    async setDefaultValues() {
      this.tableData = Array.from({ length: 6 }, () => Array(6).fill(1))
    ...
    async collectAndSubmit() {
      ...
      if (userStore.resultFlag[13] === true) {
...
<template>
  ...
  <el-row class="resultFlag">
    <el-col v-if="userStore.resultFlag[13]">
